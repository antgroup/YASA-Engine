...
const defaultFilter = (nd, prop, from) => {
   ...
   * @param obj
   */
  function objHasCallExpressionOrBinaryExpressionOrTag(obj) {
    if (!obj) {
      return false
    }
    if (
 ...
 * @param options
 */
function annotateAST(node, options) {
  sourcefile = null
  if (options) {
 ...
 */
function satisfy(node, f, filter, visited, multiMatch, maxdepth, satisfyCallback) {
  const res = []
 ...
 */
function hasTag(symVal, targetAttribute) {
   ...
   * @param visited
   */
  function hasTagRec(symVal, targetAttribute, stack, visited) {
    if (!symVal) {
      return false
    }
 ...
 * @returns {boolean}
 */
function findTag(node, attribute, multiMatch) {
...
class ASTQuery {
  ...
  findAll(match, prune) {
    ...
    function _find(subNode) {
      ...
      if (Array.isArray(subNode)) {
        subNode.forEach((s) => res.push(..._find(s)))
        return res
      }
   ...
   * @param typeName
   */
  findAllByType(typeName) {
    return this.findAll((node) => node.type === typeName)
  }
  ...
  toString() {
    ...
    let i = 0
    for (const node of this.nodes) {
      res += `${i++} : ${JSON.stringify(node, null, 2).replaceAll('\n', '    \n')}\n`
    }
    return res
  }
}
 ...
 * @param type
 */
function typeToQualifiedName(type) {
  switch (type.type) {
    case 'ScopedType': {
      if (type.scope) {
 ...
 * @param qid
 */
function qualifiedNameToMemberAccess(qid) {
  const ids = qid.split('.')
  let ret
