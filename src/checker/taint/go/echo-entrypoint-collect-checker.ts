  ...
  triggerAtAssignment(analyzer: any, scope: any, node: any, state: any, info: any) {
    ...
    switch (property.name) {
      case "HTTPErrorHandler":
        rvalueObjs.forEach(obj => processEntryPointAndTaintSource(analyzer, state, processedRouteRegistry, obj, "0, 1"));
        break;
      case "Binder":
        rvalueObjs.forEach(obj => processEntryPointAndTaintSource(analyzer, state, processedRouteRegistry, obj.field.Bind, "1"));
        break;
      case "Renderer":
        rvalueObjs.forEach(obj => processEntryPointAndTaintSource(analyzer, state, processedRouteRegistry, obj.field.Render, "3"));
        break;
      case "Filesystem":
        rvalueObjs.forEach(obj => processEntryPointAndTaintSource(analyzer, state, processedRouteRegistry, obj.field.Open, "0"));
        break;
    }
  }
    ...
    switch (symbol.expression.name) {
      ...
      case "MiddlewareWithConfig":
        flattenUnionValues([symbol.arguments[0]]).forEach(middlewareConfig => {
          ...
          if (!store) return;
          [store.field["Get"], store.field["New"], store.field["Save"]].filter(v => v).forEach(v => {
