 ...
 * @param root0.kind
 */
function markTaintSource(unit, { path, kind }) {
 ...
 */
function introduceTaintAtIdentifier(node, res, sourceScopeVal) {
  if (!BasicRuleHandler.getPreprocessReady()) {
    return
  }
 ...
 */
function matchAndMark(paths, scp, rule, mark_cb, createIfNotExists) {
  if (paths?.length === 0) {
    mark_cb(scp, rule)
    return
  }
 ...
 * @param sourceScopeVal
 */
function introduceTaintAtIdentifierDirect(node, res, sourceScopeVal) {
  if (!BasicRuleHandler.getPreprocessReady()) {
    return
  }
  if (sourceScopeVal) {
 ...
 * @param rule
 */
function getArrayElementsByRule(array, rule) {
 ...
 * @param sourceKind
 */
function introduceFuncArgTaintBySelfCollection(entryPoint, state, analyzer, rule, sourceKind) {
