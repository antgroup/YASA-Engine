...
class OutputStrategyAutoRegister {
  ...
  autoRegisterAllStrategies() {
    try {
      ...
      const jsFiles = glob.sync('*.js', {
        ...
        absolute: true,
        ignore: ['**/node_modules/**', '**/OutputStrategyAutoRegister.js', '**/OutputStrategy.js'],
      })
   ...
   * @param filePath
   */
  registerStrategyFromFile(filePath) {
    try {
   ...
   * @param moduleExports
   */
  findStrategyClass(moduleExports) {
    // 优先检查默认导出
    ...
    const exportedClasses = Object.values(moduleExports).filter(
      (exportItem) => typeof exportItem === 'function' && /^[A-Z]/.test(exportItem.name)
    )

    return exportedClasses.length > 0 ? exportedClasses[0] : null
  }
   ...
   * @param StrategyClass
   */
  isSubclassOfOutputStrategy(StrategyClass) {
