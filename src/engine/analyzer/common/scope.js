...
class Scope {
   ...
   */
  static createSubScope(name, scope, scopeName) {
    let id = name
    if (!id) {
      id = '_scope'
    }
   ...
   * @returns {*}
   */
  static getDefScopeRec(scope, node, limit) {
    if (!node || !limit) {
      return scope
    }
    switch (node.type) {
   ...
   * @param limit
   */
  getDefScopeRec(scope, node, limit) {
    return Scope.getDefScope(scope, node, limit)
  }
   ...
   */
  createIdentifierFieldValue(identifier, scope) {
    const index =
   ...
   * @returns {{vtype: string, id: *, value: {}, ast: null, parent: *}}
   */
  createVarDeclarationScope(decl, scope) {
    const id = decl.name
  ...
  createFuncScope(node, scope) {
    ...
    if (fclos && node.parameters) {
      ...
      if (targetQid && this.funcSymbolTable && typeof this.funcSymbolTable === 'object') {
        this.funcSymbolTable[targetQid] = fclos
      }
   ...
   * @returns {string}
   */
  formatScope(scope, delimit) {
    //		return JSON.stringify(scope, JSON_scope_replacer, 2);
    return ((cache = []), JSON.stringify(scope, JSON_scope_replacer, delimit))
  }
