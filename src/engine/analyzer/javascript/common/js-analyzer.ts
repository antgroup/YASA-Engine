...
class JsAnalyzer extends Analyzer {
   ...
   * @returns Unit
   */
  initModuleScope(node: any, file: any) {
    // init for module
  ...
  processTryStatement(scope: any, node: any, state: any) {
    ...
    if (node.handlers && node.handlers.length > 0) {
      ...
      const subScope = JsAnalyzer.createSubScope(
  ...
  processAssignmentExpression(scope: any, node: any, state: any) {
    ...
    if (res && res?.hasTagRec && node?.operator !== '=' && typeof res?.trace === 'undefined') {
      ...
      const trace = {
        ...
        node,
        tag: 'Var Pass:',
        affectedNodeName: (res as any).left._sid,
      }
      // this.processAssignmentToBinary(res)
      ;(res as any).trace = undefined
    }
    return res
  }
  ...
  processConditionalExpression(scope: any, node: any, state: any) {
    ...
    if (
      typeof (res as any).field !== 'undefined' &&
      (Array.isArray((res as any).field) || Object.getOwnPropertyNames((res as any).field).length !== 0) &&
      (res as any).hasTagRec !== true
    ) {
      try {
        ;(res as any).field.forEach((arg: any) => {
          if (arg.hasTagRec) {
            ;(res as any).hasTagRec = true
  ...
  processVariableDeclaration(scope: any, node: any, state: any) {
    ...
    if ((res as any)?.trace) {
      if (Array.isArray((res as any).trace)) {
        const traceDeduplication: any[] = []
        let flag = 0
        for (const argT in (res as any).trace) {
          if (argT === '0') {
            traceDeduplication[flag] = (res as any).trace[argT]
            flag++
          } else {
            if ((res as any).trace[argT].tag === 'Field: ') {
              continue
            }
            const tem = traceDeduplication[flag - 1]
            if (
              tem.file === (res as any).trace[argT].file &&
              tem.tag === (res as any).trace[argT].tag &&
              JSON.stringify(tem.line) === JSON.stringify((res as any).trace[argT].line)
            ) {
              if (tem.affectedNodeName === (res as any).trace[argT].affectedNodeName) {
                continue
        ...
        ;(res as any).trace = traceDeduplication
      }
    }

    if ((res as any)?.vtype === 'union') {
      if ((res as any).field[0]?.hasTagRec || (res as any).field[1]?.hasTagRec) {
        ;(res as any).hasTagRec = true
      }
    }
    return res
  ...
  processSpreadElement(scope: any, node: any, state: any) {
    ...
    if (res) {
      ...
      if (scope?.ast?.type === 'ObjectExpression') {
        if ((scope as any).hasTagRec !== true) {
          ;(scope as any).hasTagRec = (res as any).hasTagRec
        }
        if ((scope as any).hasTagRec) {
          if (Array.isArray((scope as any).field)) {
            ;(scope as any).field.push(res)
          } else {
            let flag = 0
            let tmp = `YASATmp${flag}`
            while ((scope as any).field[tmp]) {
              flag++
              tmp = `YASATmp${flag}`
            }
            ;(scope as any).field[tmp] = res
          }
        }
      }
    }
    ...
    if ((res as any).value) {
      for (const val in (res as any).value) {
        if (
          ...
          this.executeCall((res as any).value[val].fdef, (res as any).value[val], argvalues, state, scope)
        }
      }
    }
    return res
  ...
  processReturnStatement(scope: any, node: any, state: any) {
    ...
    if ((node as any).isYield && (retVal as any)._sid === 'Promise') {
