...
const BINARY_OPERATOR_PROCESS_MAP = {
  '&&': processAndOperator,
  '||': processOrOperator,
 ...
 * @param state
 */
function processBinaryOperator(resValue, scope, node, state) {
  try {
 ...
 * @returns {boolean}
 */
function checkLhsAndRhsValid(resValue) {
 ...
 * @param state
 */
function processAndOperator(resValue, scope, node, state) {
  ...
  if (leftValue?.vtype === 'primitive') {
    if (_.has(leftValue, 'raw_value') && leftValue?.raw_value != null) {
 ...
 * @param state
 */
function processOrOperator(resValue, scope, node, state) {
  const leftValue = resValue.left
  const rightValue = resValue.right
 ...
 * @param state
 */
function processNullMergeOperator(resValue, scope, node, state) {
  const leftValue = resValue.left
  const rightValue = resValue.right
  // 重点只关注左值是否为空
 ...
 * @returns {*}
 */
function processUnknownOperator(resValue, scope, node, state) {
  return resValue
}
