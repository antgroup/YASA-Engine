...
class EggAnalyzer extends JsAnalyzer {
   ...
   * @param state
   */
  replaceCtxInFunctionParams(astNode, argValues, entryPointSymVal, valExport, state) {
  ...
  scanModules(dir, isReScan = false) {
    ...
    if (configContents && configContents.length > 0) {
      for (const conf of configContents) {
        ...
        Initializer.assignConfig(this.topScopeTem || this.topScope, config_val)
      }
    }
   ...
   * @param modClos
   */
  processModuleDirect(ast, filename, modClos) {
    const res = super.processModuleDirect(ast, filename, modClos)
    // merge default into parent
    if (res?.field?.default && typeof res.field?.default !== undefined && res.field?.default?.vtype !== 'fclos') {
