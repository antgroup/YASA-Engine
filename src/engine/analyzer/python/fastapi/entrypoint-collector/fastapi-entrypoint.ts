...
function extractLiteralString(node: any): string | null {
  ...
  //如果node的类型是AssignmentExpression（新版本UAST），并且operator是'='，则递归调用extractLiteralString
  if (node.type === 'AssignmentExpression' && node.operator === '=' && node.right) {
    return extractLiteralString(node.right)
  }
...
function findFastApiEntryPointAndSource(filenameAstObj: FilenameAstMap, dir: string) {
  ...
  for (const filename in filenameAstObj) {
    ...
    for (const obj of body) {
      ...
      // 支持VariableDeclaration（旧版本UAST）和AssignmentExpression（新版本UAST）
      ...
      if (obj.type === 'VariableDeclaration') {
        ...
        init = obj.init
      } else if (obj.type === 'AssignmentExpression' && obj.operator === '=') {
        // 新版本UAST听说会被解析为AssignmentExpression
        if (obj.left?.type === 'Identifier') {
          varName = obj.left.name
        }
        init = obj.right
      } else {
        continue
        ...
        if (Array.isArray(init.arguments)) {
          for (const arg of init.arguments) {
            ...
            if (arg.type === 'VariableDeclaration' && arg.id?.name === 'prefix') {
              //arg.init是变量声明，例如prefix="/users"，那么arg.init就是"/users"
              const value = extractLiteralString(arg.init)
              //value
              if (value) prefix = value
            }
            // 支持新版本UAST据说可能被解析为AssignmentExpression
            if (arg.type === 'AssignmentExpression' && arg.operator === '=') {
              if (arg.left?.type === 'Identifier' && arg.left.name === 'prefix') {
                const value = extractLiteralString(arg.right)
                if (value) prefix = value
              }
            }
          }
    ...
    for (const obj of body) {
      ...
      let includePrefix = ''
      // 遍历args，args是[user_router, prefix="/users"]
      for (let i = 1; i < args.length; i += 1) {
        const arg = args[i]
        // 支持VariableDeclaration（旧版本UAST）
        ...
        // 支持AssignmentExpression听说有，但实际没有的情况
        if (arg?.type === 'AssignmentExpression' && arg.operator === '=') {
          if (arg.left?.type === 'Identifier' && arg.left.name === 'prefix') {
            const val = extractLiteralString(arg.right)
            if (val) includePrefix = val
          }
        }
      }
    ...
    for (const obj of body) {
      ...
      for (const decoratorObj of obj._meta.decorators) {
        ...
        //传入的decoratorObj.arguments?.[0]是字面量字符串，例如"/users/{name}"，那么routePath就是"/users/{name}"
