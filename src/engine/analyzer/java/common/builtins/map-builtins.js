...
class Map extends Collection {
   ...
   * @constructor
   */
  static Map(_this, argvalues, state, node, scope) {
   ...
   * @param scope
   */
  static compute(fclos, argvalues, state, node, scope) {
   ...
   * @param scope
   */
  static computeIfAbsent(fclos, argvalues, state, node, scope) {
   ...
   * @param scope
   */
  static computeIfPresent(fclos, argvalues, state, node, scope) {
    return Map.get(fclos, argvalues, state, node, scope)
  }
   ...
   * @param scope
   */
  static containsKey(fclos, argvalues, state, node, scope) {
    return new UndefinedValue()
  }
   ...
   * @param scope
   */
  static containsValue(fclos, argvalues, state, node, scope) {
    return new UndefinedValue()
  }
   ...
   * @param scope
   */
  static entrySet(fclos, argvalues, state, node, scope) {
   ...
   * @param scope
   */
  static forEach(fclos, argvalues, state, node, scope) {}
   ...
   * @param scope
   */
  static getOrDefault(fclos, argvalues, state, node, scope) {
   ...
   * @param scope
   */
  static keySet(fclos, argvalues, state, node, scope) {
    const _this = fclos.getThis()
    if (!_this) {
      return new UndefinedValue()
    }
   ...
   * @param scope
   */
  static merge(fclos, argvalues, state, node, scope) {
    const _this = fclos.getThis()
   ...
   * @param scope
   */
  static put(fclos, argvalues, state, node, scope) {
    const _this = fclos.getThis()
   ...
   * @param scope
   */
  static putAll(fclos, argvalues, state, node, scope) {
    const _this = fclos.getThis()
    if (!_this || !argvalues || argvalues.length === 0) {
      return
    }
   ...
   * @param scope
   */
  static putIfAbsent(fclos, argvalues, state, node, scope) {
    const _this = fclos.getThis()
    if (!_this || !argvalues || argvalues.length < 2) {
      return new UndefinedValue()
    }
  ...
  static remove(fclos, argvalues, state, node, scope) {
    ...
    if (Array.isArray(entryValue.field) && entryValue.field.length === 2) {
      ...
      if (argvalues.length === 1) {
        ...
        delete _this.field[keyRef]
        return value
      } else if (argvalues.length === 2 && value?._qid === argvalues[1]._qid) {
        keyRefSet.delete(keyRef)
        delete _this.field[keyRef]
        return new UndefinedValue()
      }
   ...
   * @param scope
   */
  static replace(fclos, argvalues, state, node, scope) {
    const _this = fclos.getThis()
    if (!_this || !argvalues || argvalues.length < 2) {
      return
    }
    ...
    if (Array.isArray(entryValue.field) && entryValue.field.length === 2) {
      ...
      if (argvalues.length === 2) {
        entryValue.field[1] = argvalues[1]
        return value
      } else if (argvalues.length === 3 && value?._qid === argvalues[1]._qid) {
   ...
   * @param scope
   */
  static values(fclos, argvalues, state, node, scope) {
    const _this = fclos.getThis()
    if (!_this) {
      return new UndefinedValue()
    }
