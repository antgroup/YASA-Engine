...
class SpringAnalyzer extends JavaAnalyzer {
  ...
  processVariableDeclaration(scope, node, state) {
    ...
    if (node?._meta?.modifiers && Array.isArray(node?._meta?.modifiers)) {
      const decoratorArray = node?._meta?.modifiers.filter((item) => item.startsWith('@'))
      let isBeanReferenceByName = false
      let isBeanReferenceByClass = false
      let matchedDecorator = ''
      let decoratorMeta = ''
      const indexByName = this.beanReferenceAnnotationByName.findIndex((decorator) => {
        const matchingItem = decoratorArray.find((item) => item.includes(decorator))
        if (matchingItem) {
      ...
      if (indexByName !== -1) {
        ...
        const indexByClass = this.beanReferenceAnnotationByClass.findIndex((decorator) => {
          const matchingItem = decoratorArray.find((item) => item.includes(decorator))
          if (matchingItem) {
            decoratorMeta = matchingItem
            return true
      ...
      if (isBeanReferenceByClass && matchedDecorator !== '' && decoratorMeta !== '' && !hasBeanInject) {
        ...
        if (targetClassName) {
          hasBeanInject = this.injectBeanByClass(targetClassName, node)
        }
      }
    }
    // 同package下无注解形式
    ...
    if (node._meta?.modifiers && Array.isArray(node._meta?.modifiers)) {
      ...
      for (const modifier of node._meta?.modifiers) {
        ...
        if (
          ...
          this.beanServiceAnnotationOnFunction.some((anno) => modifier.includes(anno))
        ) {
          isBeanService = true
          const regex = /name\s*=\s*"([^"]*)"/
  ...
  processClassDefinition(scope, node, state) {
    ...
    if (node._meta?.annotations && Array.isArray(node._meta?.annotations)) {
      for (const annotation of node._meta?.annotations) {
        ...
        if (
          this.beanServiceAnnotationOnClass.some((anno) => AstUtil.prettyPrintAST(annotation).includes(anno.slice(1)))
        ) {
          isBeanService = true
          beanName = this.transformBeanNameVariable(node.id?.name)
   ...
   * @param node
   */
  injectBeanByName(beanName, node) {
    if (
      beanName &&
      beanName !== '' &&
   ...
   * @param node
   */
  injectBeanByClass(targetClassName, node) {
   ...
   * @param variable
   */
  transformBeanNameVariable(variable) {
    // 检查是否是字符串
