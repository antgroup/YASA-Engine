 ...
 * @param options
 */
function parseJavaScript(code: any, options: Record<string, any>) {
  ...
  if (!ast) {
    Statistics.parsingTime += new Date().getTime() - parsingStart
 ...
 * @param options
 */
function parseJavaScriptRaw(code: any, options: Record<string, any>) {
  const JSAstBuilder = require('./javascript/js-ast-builder')
  const parsingStart = new Date().getTime()
  const ast = JSAstBuilder.parse(code, { sanity: options.sanity, sourcefile: options.sourcefile })
  if (!ast) {
 ...
 * @param options
 */
function parseJavaParsing(code: any, options: Record<string, any>) {
 ...
 * @param options
 */
function parseJavaRaw(code: any, options: Record<string, any>) {
  const JavaAstBuilder = require('./java/java-ast-builder')
  const parsingStart = new Date().getTime()
  const ast = JavaAstBuilder.parse(code, {
    sanity: options.sanity,
 ...
 * @returns {*}
 */
function parseCode(code: any, options: Record<string, any>) {
  try {
    if (options) {
      switch (options.language) {
        ...
        case 'java':
          return parseJavaParsing(code, options)
        case 'python':
    ...
    Errors.ParseError(`[${err_location_tip}] parse failed, err: ${(e as Error).toString()}`)
  }
}
 ...
 * @param options
 */
function parseCodeRaw(filepath: any, code: any, options: Record<string, any>) {
  try {
    if (options) {
      switch (options.language) {
        case 'js':
        ...
        case 'golang':
          ...
          return GoParser.parsePackage(filepath, options)
        case 'java':
    ...
    const err_location_tip = options && options.sourcefile ? options.sourcefile : `code snippet: ${code.substr(0, 70)}`
    Errors.ParseError(`[${err_location_tip}] parseRaw failed, err: ${(e as Error).toString()}`)
  }
}
 ...
 * @param options
 */
async function parseDirectory(dir: any, options: Record<string, any>) {
  try {
    if (options) {
      ...
      switch (options.language) {
        ...
        case 'golang': {
          const goUast = await GoParser.parsePackage(dir, options)
          processGoUast(goUast, options)
          const uastFile = fs.statSync('./uast.json') // 获取文件/目录状态
          if (uastFile.isFile()) {
            fs.unlink('./uast.json', (err: any) => {
              if (err) {
                handleException(
                  err,
    ...
    Errors.ParseError(`[${dir}] parseDirectory failed, err: ${(e as Error).toString()}`)
  }
}
 ...
 * @param options
 */
function processGoUast(goUast: any, options: Record<string, any>) {
   ...
   * @param parentPath
   */
  function deepSearch(obj: any, parentPath = '') {
    ...
    for (const [key, value] of Object.entries(obj)) {
      ...
      if (typeof key === 'string' && key.endsWith('.go') && value && typeof value === 'object') {
        ...
        if (
          ...
          (value as any).node.type === 'CompileUnit'
        ) {
          // 创建没有parent的node副本
          const nodeWithoutParent = removeParentProperty(JSON.parse(JSON.stringify((value as any).node)))
  ...
  function removeParentProperty(obj: any): any {
    ...
    if (Array.isArray(obj)) {
      return obj.map((item: any) => removeParentProperty(item))
    }
 ...
 */
function processPythonUast(pythonUast: any, options: Record<string, any>) {
   ...
   */
  function traverseAndWrite(obj: any, basePath = '') {
    if (!obj || typeof obj !== 'object') {
      return
    }
    ...
    for (const [key, value] of Object.entries(obj)) {
      ...
      try {
        ...
        handleException(
          ...
          `写入文件失败: ${filePath}, 错误: ${(error as Error).message}`
        )
      }
    }
  }
