...
"""
    ...
    @pytest.mark.asyncio
    async def test_one_line_method(self, tmp_path):
        """单行方法的方法范围和命中行应准确"""
        content = """\
package com.example;
public class Compact {
    public void run() { Runtime.exec("cmd"); }
}
"""
        _create_file(tmp_path, "src/Compact.java", content)
        ...
        assert match["containing_method"] == "com.example.Compact.run"
        assert match["method_start_line"] == 3
        assert match["method_end_line"] == 3
...
class TestExclusion:
    ...
    async def test_not_in_comment(self, tmp_path):
        ...
        assert result["results"][0]["match_line"] == 7

    @pytest.mark.asyncio
    async def test_comment_marker_inside_string(self, tmp_path):
        """字符串中的 // 不能被误判为行注释"""
        content = """\
package com.example;
...
"""
        _create_file(tmp_path, "src/UrlTest.java", content)

        result = await search_code_by_body_pattern(
            pattern=r"example\.test/api",
            repository_path=str(tmp_path),
        )

        assert result["total_matches"] == 1
        assert result["results"][0]["containing_method"] == "com.example.UrlTest.endpoint"

    @pytest.mark.asyncio
    async def test_braces_in_literals_do_not_change_method_range(self, tmp_path):
        """字符串、字符和 text block 中的花括号不参与结构计算"""
        content = '''\
package com.example;
public class LiteralTest {
    public void render() {
        String value = "http://example.test/{id}";
        char brace = '}';
        String json = """
            { "enabled": true }
            """;
...
'''
        _create_file(tmp_path, "src/LiteralTest.java", content)
        ...
        assert result["total_matches"] == 1
        match = result["results"][0]
        assert match["containing_method"] == "com.example.LiteralTest.render"
        assert match["method_end_line"] == 10
...
"""
    ...
    @pytest.mark.asyncio
    async def test_named_inner_class_method_fqn(self, tmp_path):
        """具名内部类的方法应使用内部类全限定名"""
        content = """\
package com.example;
public class Outer {
...
"""
        _create_file(tmp_path, "src/Outer.java", content)
        ...
        assert result["total_matches"] == 1
        assert result["results"][0]["containing_method"] == "com.example.Outer.Inner.run"
...
class TestPathPrefix:
    ...
    @pytest.mark.asyncio
    async def test_path_prefix_cannot_escape_repository(self, tmp_path):
        """path_prefix 不能越过仓库根目录"""
        result = await search_code_by_body_pattern(
            pattern="test",
            repository_path=str(tmp_path),
            path_prefix="../",
